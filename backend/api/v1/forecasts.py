...
async def generate_ai_forecast(
    ...
    metric_resp = await db.table("dim_metric").select("code, name").eq("id", metric_id).execute()
    ...
    history_resp = await db.table("fact_kpis")\
    ...
    insert_resp = await db.table("kpi_forecasts").insert(forecast_data).execute()
