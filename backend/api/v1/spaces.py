...
async def get_space(space_id: int, db=Depends(get_db)):
    response = db.table("dim_space").select("*").eq("id", space_id).execute()
...
async def create_space(data: SpaceCreate, db=Depends(get_db)):
    response = db.table("dim_space").insert(data.model_dump(exclude_unset=True)).execute()
    if not response.data:
...
async def update_space(
    space_id: int, data: SpaceUpdate, db=Depends(get_db)
):
    update_data = data.model_dump(exclude_unset=True)
    if not update_data:
        return await get_space(space_id, db)
        
    response = db.table("dim_space").update(update_data).eq("id", space_id).execute()
    if not response.data:
        raise HTTPException(status_code=404, detail="Space not found")
    return response.data[0]
