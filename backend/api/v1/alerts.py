...
async def get_alert(alert_id: int, db=Depends(get_db)):
    response = db.table("alerts").select("*").eq("id", alert_id).execute()
...
async def create_alert(data: AlertCreate, db=Depends(get_db)):
    insert_data = data.model_dump(exclude_unset=True)
    insert_data["created_at"] = datetime.utcnow().isoformat()
    response = db.table("alerts").insert(insert_data).execute()
    if not response.data:
...
async def update_alert(
    ...
    response = db.table("alerts").update(update_data).eq("id", alert_id).execute()
    if not response.data:
        raise HTTPException(status_code=404, detail="Alert not found")
    return response.data[0]
...
async def explain_alert(alert_id: int, db=Depends(get_db)):
    ...
    alert_resp = db.table("alerts").select("*, dim_institution(code), dim_metric(code)").eq("id", alert_id).execute()
    ...
    update_resp = db.table("alerts").update(update_data).eq("id", alert_id).execute()
