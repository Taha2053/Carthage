...
async def get_research_project(project_id: int, db=Depends(get_db)):
    response = db.table("dim_research_project").select("*").eq("id", project_id).execute()
...
async def create_research_project(data: ResearchProjectCreate, db=Depends(get_db)):
    response = db.table("dim_research_project").insert(data.model_dump(exclude_unset=True)).execute()
    if not response.data:
...
async def update_research_project(
    project_id: int, data: ResearchProjectUpdate, db=Depends(get_db)
):
    update_data = data.model_dump(exclude_unset=True)
    if not update_data:
        return await get_research_project(project_id, db)
        
    response = db.table("dim_research_project").update(update_data).eq("id", project_id).execute()
    if not response.data:
        raise HTTPException(status_code=404, detail="Research project not found")
    return response.data[0]
