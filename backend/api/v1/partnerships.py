...
async def get_partnership(partnership_id: int, db: AsyncClient = Depends(get_db)):
    response = await db.table("dim_partnership").select("*").eq("id", partnership_id).execute()
...
async def create_partnership(data: PartnershipCreate, db: AsyncClient = Depends(get_db)):
    response = await db.table("dim_partnership").insert(data.model_dump(exclude_unset=True)).execute()
    if not response.data:
...
async def update_partnership(
    partnership_id: int, data: PartnershipUpdate, db: AsyncClient = Depends(get_db)
):
    update_data = data.model_dump(exclude_unset=True)
    if not update_data:
        return await get_partnership(partnership_id, db)
        
    response = await db.table("dim_partnership").update(update_data).eq("id", partnership_id).execute()
    if not response.data:
        raise HTTPException(status_code=404, detail="Partnership not found")
    return response.data[0]
