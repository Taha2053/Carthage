...
async def test_endpoints():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            ...
            print(f"OK GET / -> {r.status_code}" if ok else f"FAIL GET / -> {r.status_code}")
            results.append(ok)
        except Exception as e:
            print("FAIL GET / -> ERROR")
        ...
        try:
            ...
            ok = r.status_code == 200
            print(f"OK GET /api/v1/health -> {r.status_code}" if ok else f"FAIL GET /api/v1/health -> {r.status_code}")
            results.append(ok)
        except Exception as e:
            print("FAIL GET /api/v1/health -> ERROR")
            results.append(False)
        ...
        for method, path in endpoints:
            try:
                ...
                status_str = "OK" if 200 <= r.status_code < 500 else "FAIL"
                ...
                print(f"FAIL {method} {path} -> ERROR")
                results.append(False)
