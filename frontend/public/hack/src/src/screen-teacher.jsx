          ...
          <div className="mt-6">
            ...
            {sortedRisk.map((s,idx)=>(
              <div key={s.id} className="grid grid-cols-12 gap-3 py-3 border-b border-rule items-center hover:bg-paper2/60">
                ...
                <div className="col-span-2 flex items-center gap-2"><Bar value={s.presence} color={s.presence>=80?'#3F6B3F':s.presence>=65?'#13110E':'#A8341E'} height={4}/><span className="num text-[12px] w-9 text-right">{s.presence}%</span></div>
                <div className="col-span-1 flex items-center gap-2"><Bar value={s.ontime} color={s.ontime>=80?'#3F6B3F':s.ontime>=65?'#13110E':'#A8341E'} height={4}/><span className="num text-[12px] w-9 text-right">{s.ontime}%</span></div>
                <span className="col-span-1 num text-[18px] text-right" style={{color: s.score>=85?'#3F6B3F':s.score>=72?'#13110E':s.score>=60?'#B8842B':'#A8341E'}}>{s.score}</span>
                    ...
                    {list.map(s=>(
                      <div key={s.id} className="bg-paper border border-rule rounded p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="num text-[16px]" style={{color: s.score>=85?'#3F6B3F':s.score>=72?'#13110E':s.score>=60?'#B8842B':'#A8341E'}}>{s.score}</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <div><div className="text-[9px] uppercase tracking-[0.14em] text-ink3">Pres.</div><Bar value={s.presence} color="#13110E" height={3}/></div>
                          <div><div className="text-[9px] uppercase tracking-[0.14em] text-ink3">On-time</div><Bar value={s.ontime} color="#13110E" height={3}/></div>
                        </div>
                      </div>
                    ))}
                  </div>
