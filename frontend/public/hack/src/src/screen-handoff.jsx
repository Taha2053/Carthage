...
export function PulseLine({ score, w = 240, h = 32, animated = false }: { score: number; w?: number; h?: number; animated?: boolean; }) {
  const c = score >= 85 ? '#3F6B3F' : score >= 72 ? '#13110E' : score >= 60 ? '#B8842B' : '#A8341E';
  ...
  return (
    <svg width={w} height={h} viewBox={\`0 0 \${w} \${h}\`} className="block">
      <line x1="0" y1={h/2} x2={w} y2={h/2} stroke="#E2D9C8" strokeWidth="0.5" strokeDasharray="2 2"/>
...
export function InstitutionTrend({ data, color = '#13110E' }: { data: { wk: string; v: number }[]; color?: string }) {
  return (
    <ResponsiveContainer width="100%" height={280}>
      <LineChart data={data} margin={{ top: 18, right: 16, bottom: 26, left: 36 }}>
        <CartesianGrid stroke="#E2D9C8" strokeDasharray="2 3" vertical={false} />
        <XAxis dataKey="wk" stroke="#7A7268" fontFamily="JetBrains Mono" fontSize={10} tickLine={false} axisLine={false} />
        <YAxis stroke="#7A7268" fontFamily="JetBrains Mono" fontSize={10} tickLine={false} axisLine={false} />
        <Tooltip contentStyle={{ background: '#13110E', color: '#F6F2EA', border: 'none', fontFamily: 'JetBrains Mono', fontSize: 12 }} />
