...
const ScreenInstitution = ({ setActive }) => {
  ...
  const tabs = [
    { id:'composite', label:'Composite', data: series.composite, color:'#003966' },
    { id:'attendance',label:'Attendance', data: series.attendance, color:'#3F6B3F' },
    { id:'ontime',    label:'On-time submission', data: series.ontime, color:'#EE771D' },
    { id:'pass',      label:'Pass rate', data: series.pass, color:'#B85C0D' },
  ];
    ...
    <div className="screen" data-active="true">
      <div className="max-w-[1400px] mx-auto px-8 pt-10 pb-20">
        ...
        <div className="grid grid-cols-12 gap-3 mt-12">
          {[
            { k:'Students',    v:'4,120', s:'+38 this term', spark: window.NABD.ts(33, 4000, 30), color:'#003966', unit:'enrolled' },
            { k:'Attendance',  v:'92.4%', s:'+1.2 wk',       spark: series.attendance,            color:'#3F6B3F', unit:'avg, all cohorts' },
            { k:'On-time',     v:'81.6%', s:'+4.1 wk',       spark: series.ontime,                color:'#EE771D', unit:'submission window' },
            { k:'Pass rate',   v:'89.2%', s:'+0.6 wk',       spark: series.pass,                  color:'#B85C0D', unit:'last assessment' },
          ].map(t=>(
            ...
            <div className="mt-4 space-y-3.5">
              {window.NABD.ENG_DEPTS.sort((a,b)=>b.value-a.value).map(d=>(
                <div key={d.name}>
                  ...
                  <div className="mt-1.5"><Bar value={d.value} color={d.value>=88?'#3F6B3F':d.value>=80?'#003966':'#B8842B'} height={3}/></div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-10 grid grid-cols-12 gap-8">
          ...
          <div className="col-span-12 lg:col-span-5 bg-paper2/50 border border-rule rounded p-7">
            ...
            <div className="mt-5 dashed-box rounded p-8 text-center">
              ...
              <div className="font-display text-[22px] tracking-tightish mt-3">Drop a CSV here, or <span className="text-accent ul-link" style={{textDecorationColor:'#EE771D'}}>browse</span></div>
