...
let MOCK_DATA: Department[] = [
  { id: 1, name: 'Informatique', code: 'DEV', headOfDepartment: 'Dr. Karim Mansouri', contactEmail: 'info@fsegt.tn' },
  { id: 2, name: 'Économie', code: 'ECO', headOfDepartment: 'Prof. Jamil', contactEmail: 'eco@fsegt.tn' },
]
...
export default function AdminDepartments() {
  ...
  const [currentEdit, setCurrentEdit] = useState<Partial<Department> | null>(null)
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault()
    if (!currentEdit?.name || !currentEdit?.code) {
      toast.error(t('common.error', 'Erreur : champs obligatoires manquants.'))
      return
    }
    if (currentEdit.id) {
      // Update
      MOCK_DATA = MOCK_DATA.map((d) => (d.id === currentEdit.id ? { ...d, ...currentEdit } as Department : d))
    } else {
      // Create
      MOCK_DATA.push({ ...currentEdit, id: Date.now() } as Department)
    }
    setItems([...MOCK_DATA])
    ...
    <div className="space-y-8 py-6 px-6 max-w-[1400px] mx-auto">
      <div className="flex items-start justify-between fade-up">
        <div>
          ...
          <h1 className="font-display text-4xl gold-shimmer tracking-tighter2">{t('admin.departments.title', 'Départements')}</h1>
        </div>
        <button
          onClick={() => { setCurrentEdit({ name: '', code: '', headOfDepartment: '', contactEmail: '' }); setIsModalOpen(true) }}
          className="btn-primary px-5 py-2.5 rounded-lg text-sm font-medium flex items-center gap-2"
        >
          <Plus className="w-4 h-4" /> {t('admin.departments.add', 'Ajouter un département')}
        </button>
      </div>
        ...
        <div className="rounded-xl border border-rule overflow-hidden bg-paper shadow-sm fade-up-1">
          <table className="w-full text-sm">
            <thead className="bg-paper2 border-b border-rule">
              <tr>
                <th className="px-4 py-3 text-left text-[11px] uppercase tracking-[0.1em] text-ink3">Code</th>
                <th className="px-4 py-3 text-left text-[11px] uppercase tracking-[0.1em] text-ink3">Nom du département</th>
                <th className="px-4 py-3 text-left text-[11px] uppercase tracking-[0.1em] text-ink3">Chef de département</th>
                <th className="px-4 py-3 text-left text-[11px] uppercase tracking-[0.1em] text-ink3">Contact</th>
            ...
            <tbody>
              {items.map((b) => (
                <tr key={b.id} className="border-b border-rule last:border-0 hover:bg-paper2/50">
                  ...
                  <td className="px-4 py-3 text-ink2">{b.headOfDepartment}</td>
                  <td className="px-4 py-3 text-ink3">{b.contactEmail}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/50 backdrop-blur-sm">
          <div className="bg-paper border border-rule rounded-xl w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSave} className="p-4 space-y-4">
              <div>
                <label className="text-[11px] uppercase tracking-[0.1em] text-ink3 block mb-1">Code *</label>
                <input required value={currentEdit?.code || ''} onChange={e => setCurrentEdit({ ...currentEdit, code: e.target.value })} className="w-full rounded border-rule bg-paper2/50 px-3 py-2 text-sm focus:border-gold focus:outline-none" />
              </div>
              <div>
                <label className="text-[11px] uppercase tracking-[0.1em] text-ink3 block mb-1">Nom du département *</label>
              ...
              <div>
                <label className="text-[11px] uppercase tracking-[0.1em] text-ink3 block mb-1">Chef de département</label>
                <input value={currentEdit?.headOfDepartment || ''} onChange={e => setCurrentEdit({ ...currentEdit, headOfDepartment: e.target.value })} className="w-full rounded border-rule bg-paper2/50 px-3 py-2 text-sm focus:border-gold focus:outline-none" />
              </div>
